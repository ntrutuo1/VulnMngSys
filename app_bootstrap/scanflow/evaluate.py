...
def evaluate_registry_expected(rule: dict[str, Any], actual: str) -> bool:
    if "registry_value" in rule:
        return str(rule.get("registry_value")).strip() == actual.strip()

    expected = rule.get("expected")
    if isinstance(expected, dict):
        return evaluate_numeric_expected(expected, actual)

    if isinstance(expected, (int, float)):
        return str(int(expected)) == actual.strip()

    if isinstance(expected, str):
        expected_lower = expected.strip().lower()
        if expected_lower in {"enabled", "disabled"} and actual.strip() in {"0", "1"}:
            target = "1" if expected_lower == "enabled" else "0"
            return actual.strip() == target
