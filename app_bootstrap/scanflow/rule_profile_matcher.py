...
def sorted_rule_files(profile_dir: Path) -> list[Path]:
    files = [path for path in profile_dir.glob("*.json") if path.is_file() and not is_backup_path(path)]
    return sorted(files, key=lambda path: (_rule_file_score(path), path.name.casefold()))
