...
from app_bootstrap.scanflow.views.scan_messages import (
...
def _rule_id(row: dict[str, Any]) -> str:
    return _coerce_text(row.get("id") or row.get("RuleId") or row.get("ruleId") or row.get("RuleID") or row.get("rule_id"))
...
def _expected(row: dict[str, Any]) -> str:
    return _coerce_text(row.get("expected") or row.get("Expected") or row.get("Recommended") or row.get("RecommendedValue") or row.get("recommended"))
...
class ScanView:
    ...
    def normalize_row(self, row: dict[str, Any]) -> dict[str, Any]:
        if "verdict" in row or "passed" in row:
            ...
            raw_status = _normalize_status(_coerce_text(row.get("Status") or row.get("status") or row.get("Verdict") or row.get("verdict")))
        ...
        check_type = _coerce_text(row.get("check_type") or row.get("CheckType") or row.get("checkType") or row.get("type"))
        return {
            ...
            "guidance": list(row.get("guidance") or ([] if passed else ([f"Review rule {rule_id}"] if rule_id else []))),
        }
