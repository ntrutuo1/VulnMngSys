...
class ReconfigTests(unittest.TestCase):
    ...
    def test_remediation_pipeline_apply_runs_backup_execute_and_verify(self) -> None:
        ...
        self.assertEqual(
            events,
            [
                "backup:['A']:['HKLM\\\\Software\\\\VulnMngSysTest']",
                "snapshot",
                "execute:vulnmngsys_reconfig.ps1",
                "verify:Running",
            ],
        )
    ...
    def test_remediation_pipeline_rolls_back_when_script_fails(self) -> None:
        report = {
            "items": [
                {
                    "id": "A",
        ...
        self.assertFalse(payload["ok"])
        self.assertTrue(payload["autoRolledBack"])
        self.assertEqual(events, ["rollback:backup-3"])

    def test_iis_remediation_rolls_back_when_service_verification_fails(self) -> None:
        report = {
            "results": [
                {
                    "status": "FAIL",
                    ...
                    "name": "Web Deploy RCE",
                    "local_check_result": {
                        "patch_after": "2025-07-01",
                        "patch_guidance": "Install approved Web Deploy update.",
                    },
                }
            ]
        }
        ...
        class FakeBackup:
            def trigger_backup(self, selected_rule_ids, registry_paths=None):
                events.append(f"backup:{selected_rule_ids}")
                return "iis-backup"
        ...
        class FakeVerifier:
            def get_services_status(self, service_names):
                events.append(f"snapshot:{'W3SVC' in service_names}")
                return {"W3SVC": "Running"}
        ...
        with tempfile.TemporaryDirectory() as tmp:
            payload = IisRemediationPipeline(
                backup=FakeBackup(),
                verifier=FakeVerifier(),
                executor=FakeExecutor(),
            ).apply(report, Path(tmp), selected_cves=["CVE-2025-53772"])
            close_remediation_loggers()

        self.assertFalse(payload["ok"])
        self.assertTrue(payload["autoRolledBack"])
        self.assertEqual(payload["cves"], ["CVE-2025-53772"])
        self.assertEqual(
            events,
            [
                "backup:['CVE-2025-53772']",
                "snapshot:True",
                "execute:vulnmngsys_iis_cve_reconfig.ps1",
                "verify:Running",
                "rollback:iis-backup",
            ],
        )
