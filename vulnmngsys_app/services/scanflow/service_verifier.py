...
def check_webdeploy_installed() -> dict[str, Any]:
    ...
    command = (
        ...
        "if ($item) { "
        "[pscustomobject]@{ Path=$expanded; Exists=$true; Version=[string]$item.VersionInfo.FileVersion } } "
        "else { [pscustomobject]@{ Path=$expanded; Exists=$false; Version='' } } "
        "}; "
        "$reg = Get-ItemProperty -Path 'HKLM:\\SOFTWARE\\Microsoft\\IIS Extensions\\MSDeploy\\3' "
    ...
    return {
        "installed": (
            bool(services)
            or any(row.get("Exists") for row in files)
            or bool(payload.get("RegistryExists") if isinstance(payload, dict) else False)
        ),
        "services": {
            name: {
                "name": status.name,
