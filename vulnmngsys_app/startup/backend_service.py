...
from vulnmngsys_app.controllers.api_server import create_api_server
from vulnmngsys_app.controllers.msf_routes import start_msf_runtime
...
def main(argv: list[str] | None = None) -> int:
    ...
    def stop(_signum: int, _frame: object) -> None:
        ...
        if stopping:
            return
        stopping = True
        get_msf_manager().shutdown()
        threading.Thread(target=server.shutdown, name="vulnmngsys-backend-shutdown", daemon=True).start()
